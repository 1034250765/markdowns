...
- 先开发技能，再开发 agent；agent 的 MD 中声明可使用的技能。
- 技能是共享资源，subagent 不独享技能，只声明"我有哪些技能可用"。

**触发链**

```
用户提出需求 -> 主代理（Claude Code）判断由哪个 subagent 干活
           -> 委派并等待
           -> subagent 调用所需技能完成任务
           -> 结果返回主代理
```
...
- **隔离**：subagent 的独立上下文意味着它只收到任务指令本身，不带主会话历史，避免记忆混乱——和前面"子代理执行过程拆解"里京东/淘宝两个搜索代理互不可见是同一回事。

**待核实**

- 手动调用 subagent 的具体命令形式（课程转写中提到"也可手动指定"，但命令未确认）。
- agents MD 的官方字段规范（课程演示中该文件由 Claude 代写，未展示规范本身）。

> 来源：wiki/comparisons/claude-code-skill-vs-subagent.md（整理自黑马程序员 Vibe Coding 第 19 集课程转写，个别细节待核实）
