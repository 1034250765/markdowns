...
> 核心一句话：**每个进程一启动，操作系统就自动给它分配三个文件描述符（fd），编号 0、1、2，默认指向终端设备**——注意这里的"文件"不是磁盘文件，而是"一切可 read/write 的资源"的统一接口（终端、管道、磁盘文件、黑洞……都算）。这个统一接口是重定向和管道能存在的全部原因。

### "文件"这个词的三层含义（防混淆）

| 层 | 是什么 | 例子 |
|---|---|---|
| **fd（文件描述符）** | 进程内的小整数，索引到内核的打开文件表 | 0、1、2 |
| **打开文件描述**（内核结构体） | 记录"这一次打开"的状态：读写位置 offset、打开模式、操作表 | `struct file`（Linux 内核） |
| **真实资源** | fd 最终接到的那个东西 | 终端设备 `/dev/pts/1`、管道 `pipe:[1085]`、磁盘文件 `out.txt`、黑洞 `/dev/null` |

默认三个 fd 指向**终端设备文件**——它在目录树里有名字（`/dev/pts/N`），但读写它不碰磁盘，内核把 read/write 转发给终端驱动。只有重定向到普通文件时才真正落盘。这就是 VFS（虚拟文件系统）："一切皆文件"是**接口的统一**，不是**实现的统一**。
